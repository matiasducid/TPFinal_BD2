...
drop table "Clientes";
drop table "Eq_Clientes";
drop table "Eq_Ventas";
drop table "Ventas";
	...
	FOR row_temp IN cursor_temp LOOP
		...
		CASE row_temp.cod_medio_pago
			...
			ELSE
				medio = 'CHEQUE';
		END CASE;
		INSERT INTO "Eq_Ventas" VALUES(null,null,null,null,null,row_temp.fecha,
			row_temp.tiempo,row_temp.factura,row_temp.id_prod,row_temp.cantidad,
			row_temp.precio,row_temp.cliente,row_temp.nombre,medio);
	END LOOP;
END
$$
LANGUAGE plpgsql;
...
BEGIN
	FOR row_venta IN cursor_ventas LOOP
		...
		ELSE
				...
				--Directamente agregamos desde el cursor porque recorre la Eq_Ventas.
			INSERT INTO "Ventas" VALUES (row_venta."Fecha_Vta2",row_venta."Id_Tiempo2",row_venta."claveDW_Ventas",
						(SELECT nueva_clave FROM "Eq_Clientes" WHERE "Eq_Clientes".idc2=row_venta."cod_Cliente2")
...
BEGIN
	...
	INSERT INTO "tmp_V2" (
		SELECT ventasf2."Fecha_Vta2",ventasf2."Id_Tiempo2",ventasf2."id_Prod",
		ventasf2."Id_Factura2",ventasf2."cod_Cliente2",ventasf2."Nombre2",
		ventasf2.cod_medio_pago2, ventasf2.cantidad, ventasf2.precio
		FROM dblink('conexionFacturacion2a','SELECT v."Fecha_Vta",
		v."Id_Tiempo",d.cod_producto,v."Id_Factura",v."cod_Cliente",
		v."Nombre",v.cod_medio_pago, sum(d.unidad) as cantidad,
		sum(d.precio) as precio FROM "Venta" v, "Detalle_Venta" d 
		WHERE v."Id_Factura"=d."Id_Factura"  GROUP BY (v."Id_Factura",d.cod_producto) ')
		AS ventasf2("Fecha_Vta2" date,"Id_Tiempo2" integer,"id_Prod" integer,
		"Id_Factura2" integer,"cod_Cliente2" integer, "Nombre2" varchar(50),
